...
if (deck) {
  ...
  if (prevBtn) prevBtn.onclick = () => goToDeck(deckIndex - 1);
  if (nextBtn) nextBtn.onclick = () => goToDeck(deckIndex + 1);

  const playAlong = document.getElementById('play-along');
  if (playAlong) playAlong.onclick = () => goToDeck(deckIndex + 1);

  document.addEventListener('keydown', e => {
    if (document.getElementById('watch-overlay')) return;
    const tag = (e.target && e.target.tagName) || '';
    if (tag === 'INPUT' || tag === 'TEXTAREA') return;
    if (e.key === 'ArrowRight' || e.key === 'PageDown') { e.preventDefault(); goToDeck(deckIndex + 1); }
    else if (e.key === 'ArrowLeft' || e.key === 'PageUp') { e.preventDefault(); goToDeck(deckIndex - 1); }
  ...
  deck.addEventListener('wheel', e => {
    ...
    if (wheelLock) return;
    wheelLock = true;
    goToDeck(deckIndex + (e.deltaY > 0 ? 1 : -1));
